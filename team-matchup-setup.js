...
function scheduleLoad(){const sel=document.querySelector('#event-team-select');if(!sel)return;const key=sel.value;if(!CONFIG[key]){loadToken++;hidePairControls(false);return}hidePairControls(true);[40,300,900,1600].forEach((ms,i)=>setTimeout(()=>{if(document.querySelector('#event-team-select')?.value!==key)return;const shown=document.querySelector('#cornhole-seeding .team-matchup-setup')?.dataset.eventKey;if(i===0||shown!==key)load()},ms))}
