...
async function fastCornholeScore(request,env,ctx){
  ...
  try{
    ...
    const r=await fetch(`${base}/rest/v1/rpc/save_cornhole_score`,{
      ...
      body:JSON.stringify({p_match_notion_id:matchId,p_score_a:scoreA,p_score_b:scoreB})
    });
    const text=await r.text();let d={};try{d=text?JSON.parse(text):{}}catch{d={}}
    if(!r.ok)return json({error:d?.message||d?.error||`Supabase ${r.status}`},r.status>=400&&r.status<500?r.status:502);
    ...
    if(ctx?.waitUntil&&env.NOTION_API_TOKEN){
      ...
      const changed=raw.filter(x=>codes.has(x.match_code));
      ctx.waitUntil(Promise.allSettled(changed.map(x=>notionPatchMatch(x,env))));
    }
    return json({ok:true,winner:d.winner||'',loser:d.loser||'',matches:raw.map(legacyMatch),source:'supabase-fast'});
