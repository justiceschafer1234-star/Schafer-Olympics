(()=>{
  const els={
    ...
    total:document.querySelector('#stat-total'),registered:document.querySelector('#stat-registered'),unregistered:document.querySelector('#stat-unregistered'),entries:document.querySelector('#stat-entries'),average:document.querySelector('#stat-average'),
    divisionSummary:document.querySelector('#division-summary'),rows:document.querySelector('#participant-rows'),count:document.querySelector('#participant-count'),eventSummary:document.querySelector('#event-summary'),
    search:document.querySelector('#participant-search'),registrationFilter:document.querySelector('#registration-filter'),divisionFilter:document.querySelector('#division-filter'),refresh:document.querySelector('#refresh-dashboard'),
    percent:document.querySelector('#registration-percent'),ring:document.querySelector('#registration-ring'),progressLabel:document.querySelector('#registration-progress-label'),
    completeCount:document.querySelector('#registration-complete-count'),pendingCount:document.querySelector('#registration-pending-count'),registeredList:document.querySelector('#registered-list'),unregisteredList:document.querySelector('#unregistered-list'),lastUpdated:document.querySelector('#last-updated')
  };
  ...
  const isRegistered=p=>Array.isArray(p.registeredEventIds)&&p.registeredEventIds.length>0;

  function stats(){
    const total=data.participants.length;
    const registered=data.participants.filter(isRegistered).length;
    const entries=data.participants.reduce((sum,p)=>sum+(p.registeredEventIds?.length||0),0);
    ...
    els.unregistered.textContent=pending;
    els.entries.textContent=entries;
    els.average.textContent=registered?(entries/registered).toFixed(1):'0.0';
    els.completeCount.textContent=registered;
    els.pendingCount.textContent=pending;
    els.percent.textContent=`${pct}%`;
    els.progressLabel.textContent=`${registered} of ${total} participants registered`;
    els.ring.style.setProperty('--progress',`${pct*3.6}deg`);
  }
  ...
  function events(){
    ...
    els.eventSummary.innerHTML=ordered.length?ordered.map(e=>{
      const n=counts.get(e.id)||0;
      return `<article class="event-row"><div class="event-number">${esc(e.number??'–')}</div><div class="event-title"><strong>${esc(e.name)}</strong><span>${esc(e.format||'Event')}</span></div><div class="event-meter"><div class="event-meter-fill" style="width:${Math.round((n/max)*100)}%"></div></div><div class="event-total"><strong>${n}</strong><span>registered</span></div></article>`;
    }).join(''):'<p class="empty-state">No events available.</p>';
  }

  function registrationLists(){
    const map=eventMap();
    const card=p=>{
      const names=(p.registeredEventIds||[]).map(id=>map.get(id)).filter(Boolean);
      return `<div class="person-row"><div><strong>${esc(p.name)}</strong><span>${(p.divisions||[]).map(d=>esc(divisionNames[d]||d)).join(', ')||'No division'}</span></div><div class="person-meta">${names.length?`${names.length} event${names.length===1?'':'s'}`:'Needs registration'}</div></div>`;
    };
    const yes=data.participants.filter(isRegistered).sort((a,b)=>a.name.localeCompare(b.name));
    const no=data.participants.filter(p=>!isRegistered(p)).sort((a,b)=>a.name.localeCompare(b.name));
    els.registeredList.innerHTML=yes.length?yes.map(card).join(''):'<p class="empty-state">No registered participants yet.</p>';
    els.unregisteredList.innerHTML=no.length?no.map(card).join(''):'<p class="empty-state">Everyone is registered.</p>';
  }

  function render(){stats();divisions();participants();events();registrationLists();}

  function setTab(name){
    document.querySelectorAll('.tab-button').forEach(btn=>btn.classList.toggle('is-active',btn.dataset.tab===name));
  ...
  async function load(){
    ...
    try{
      ...
      els.status.textContent='Registration data is up to date.';
      setTimeout(()=>{els.status.hidden=true},1400);
  ...
  if(['overview','events','participants','registration'].includes(initial))setTab(initial);
  load();
})();
