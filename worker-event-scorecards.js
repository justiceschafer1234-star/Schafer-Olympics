...
async function loadKidsSoccer(env){const [sides,events]=await Promise.all([sb(env,'kids_soccer_sides?select=side_number,team_a,team_b,score,updated_at&order=side_number.asc'),sb(env,'olympic_events?select=notion_page_id,event,event_key,event_number,status,gold_teams,silver_teams&event_key=eq.kids-soccer&limit=1')]);const event=events[0];return{ok:true,eventId:event?.notion_page_id||'',event:{name:event?.event||'Kids Soccer',key:'kids-soccer',number:event?.event_number||2,status:event?.status||'Not Started',goldTeams:event?.gold_teams||[],silverTeams:event?.silver_teams||[]},sides}}
async function kidsSoccer(request,env){try{if(request.method==='GET')return json(await loadKidsSoccer(env));if(request.method!=='POST')return json({error:'Method not allowed'},405);let b={};try{b=await request.json()}catch{return json({error:'Invalid request.'},400)}if(!env.ADMIN_SCORE_CODE||String(b.code||'')!==String(env.ADMIN_SCORE_CODE))return json({error:'Incorrect control code.'},401);if(b.action==='configure'){try{await rpc(env,'configure_kids_soccer',{p_teams:Array.isArray(b.teams)?b.teams:[],p_force_reset:Boolean(b.forceReset)})}catch(e){const m=String(e.message||e);if(/reset the recorded score/i.test(m))return json({error:m,needsResetConfirmation:true},409);throw e}return json(await loadKidsSoccer(env))}if(b.action==='saveScore'){const n=Number(b.score);if(!Number.isInteger(n)||n<0)return json({error:'Score must be a whole number of 0 or greater.'},400);await rpc(env,'save_kids_soccer_score',{p_side_number:Number(b.sideNumber),p_score:n});return json(await loadKidsSoccer(env))}if(b.action==='clearScore'){await rpc(env,'clear_kids_soccer_score',{p_side_number:Number(b.sideNumber)});return json(await loadKidsSoccer(env))}return json({error:'Unknown action.'},400)}catch(e){return json({error:String(e?.message||e)},502)}}

export default{async fetch(request,env,ctx){const u=new URL(request.url);if(u.pathname==='/api/kids-soccer')return kidsSoccer(request,env);if(u.pathname!=='/api/event-scorecard')return app.fetch(request,env,ctx);const key=String(u.searchParams.get('eventKey')||'');try{if(request.method==='GET')return json(await load(env,key));if(request.method!=='POST')return json({error:'Method not allowed'},405);let b={};try{b=await request.json()}catch{return json({error:'Invalid request.'},400)}if(!env.ADMIN_SCORE_CODE||String(b.code||'')!==String(env.ADMIN_SCORE_CODE))return json({error:'Incorrect control code.'},401);if(b.action==='reset')return json(await save(env,key,{}));if(b.action!=='save'||!b.state||typeof b.state!=='object'||Array.isArray(b.state))return json({error:'Invalid scorecard state.'},400);return json(await save(env,key,b.state))}catch(e){return json({error:String(e?.message||e)},502)}}};
