...
async function rpc(env,name,body={}){
  ...
  if(!r.ok){const e=new Error(d?.message||d?.error||`Supabase ${r.status}`);e.status=r.status;throw e}return d;
}
...
async function saveScore(request,env){try{const b=await requireCode(request,env);const matchId=String(b.matchId||''),a=Number(b.scoreA),bb=Number(b.scoreB);if(!matchId||!Number.isFinite(a)||!Number.isFinite(bb)||a<0||bb<0||a===bb)return json({error:'Enter both teams and a non-tied score.'},400);const d=await rpc(env,'save_wiffle_ball_score',{p_match_notion_id:matchId,p_score_a:a,p_score_b:bb});return json({ok:true,winner:d.winner||'',loser:d.loser||'',matches:(d.matches||[]).map(legacyMatch),source:'supabase-fast'})}catch(e){return json({error:e.message||String(e)},e.status||502)}}

async function seed(request,env){try{const b=await requireCode(request,env);const base=cleanBase(env.SUPABASE_URL);const check=await fetch(`${base}/rest/v1/wiffle_ball_matches?select=match_code,status&status=eq.Complete`,{headers:{apikey:env.SUPABASE_SECRET_KEY}});const completed=check.ok?await check.json():[];if(completed.length&&!b.forceReset)return json({error:'Wiffle Ball already has completed games. Random seeding will reset the bracket.',needsResetConfirmation:true},409);const d=await rpc(env,'seed_wiffle_ball',{});return json({ok:true,seeds:d.seeds||[],matches:(d.matches||[]).map(legacyMatch),source:'supabase-fast'})}catch(e){return json({error:e.message||String(e)},e.status||502)}}
